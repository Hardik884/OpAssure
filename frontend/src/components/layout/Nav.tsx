...
export function Nav() {
  ...
  return (
    <>
      ...
      <nav
        aria-label="Primary"
        className="sticky top-16 z-20 hidden border-b border-white/10 bg-ink-900 sm:block"
      >
          ...
          {NAV_ITEMS.map(({ key, label, href, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                className={`flex min-h-12 flex-1 items-center justify-center gap-2 border-b-2 px-3 text-sm font-semibold transition-colors ${
                  active
      ...
      <nav
        aria-label="Primary"
        className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-ink-900 sm:hidden"
          ...
          {NAV_ITEMS.map(({ key, label, href, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                href={href}
                aria-current={active ? "page" : undefined}
                className={`flex min-h-16 flex-1 flex-col items-center justify-center gap-1 px-0.5 text-[11px] font-semibold leading-none transition-colors ${
